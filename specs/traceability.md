...
| Authorization | `security/access-control.md` | SC-SEC-*; `SecurityErrorContractIntegrationTest`; mobile API/session tests | Every enabled sensitive endpoint is tested with absent, allowed and denied roles; mobile distinguishes 401 session loss from 403 access denial | VERIFIED MVP / PROD LIFECYCLE PENDING |
