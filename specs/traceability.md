...
| Student operation QR | `ui/student-experience.md`, `features/identify-participant/*`, `features/scan-container/ui.md` | operation QR controller/widget tests; API client contract test | Participant selects delivery/return and renders the exact short-lived server payload; no local token generation | IMPLEMENTED / DEVICE VERIFY PENDING |
...
| Cafeteria navigation | `ui/cafeteria-experience.md` | role shell and dual-scan widget/controller tests | Shell defaults to real camera scan; participant QR must resolve before container QR; other data modules remain unavailable explicitly | PARTIAL |
...
| Participant operation QR | `domain/participant.md`, `features/identify-participant/*`, `features/scan-container/*` | `QrUseCaseTest`; HMAC codec tests; security HTTP workflow; Flutter controller/widget/client tests | Signed two-minute server token, explicit account binding, purpose and minimal resolution implemented; token consumption remains F5/F6 | IMPLEMENTED / DEVICE VERIFY PENDING |
| Container QR identity | `domain/container-lifecycle.md`, `features/scan-container/*`, `api/openapi-baseline.md` | codec/use-case/contract/security/PostgreSQL tests; Docker HTTP check | Signed/versioned static QR, generation rotation/revocation and server-derived operator actions implemented | IMPLEMENTED / DEVICE VERIFY PENDING |
...
| Container lifecycle | `domain/container-lifecycle.md` | `ContainerTest`; `ReturnContainerUseCaseTest`; SC-CTR-* | Normal flow persists `IN_USE → RETURNED → AVAILABLE`; wash use case remains unimplemented | NORMAL FLOW FOUNDATION IMPLEMENTED |
...
| Return domain | `domain/circulation.md` | `CirculationTest`; `ReturnContainerUseCaseTest`; `PersistenceAdapterIntegrationTest` | Return completes circulation and persists `RETURNED`; complete-wash feature remains pending | PARTIAL |
