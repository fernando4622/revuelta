...
| API error contract | `api/errors.md`, `api/openapi-baseline.md` | `GlobalExceptionHandlerTest`; `SecurityErrorContractIntegrationTest` | Malformed requests and security failures use stable `application/problem+json` with the response correlation ID; remaining endpoint failures require alignment | PARTIAL |
| Build/configuration baseline | `ops/deployment.md`, `AGENTS.md`, `ROADMAP.md` F1 | Maven Wrapper build; Flutter checks; OpenAPI lint; Trivy scan; GitHub Actions runs through `35521186422` | Canonical Maven/JDK 17 build, isolated dev seeds, configurable mobile URL and green CI workflow | VERIFIED |
